...
enum class UseColour { Auto, Yes, No };
...
public:
  LazyExpression(bool isNegated) : m_isNegated(isNegated) {}
...
public:
  constexpr unique_ptr(std::nullptr_t = nullptr) : m_ptr{} {}
  explicit constexpr unique_ptr(T *ptr) : m_ptr(ptr) {}

  template <typename U,
            typename = std::enable_if_t<std::is_base_of<T, U>::value>>
  unique_ptr(unique_ptr<U> &&from) : m_ptr(from.release()) {}
...
public:
  ...
  BenchmarkFunction(BenchmarkFunction &&that) noexcept : f(std::move(that.f)) {}
...
[[noreturn]] void throw_optimized_away_error();
...
namespace Benchmark {
struct Benchmark {
  Benchmark(std::string &&benchmarkName) : name(std::move(benchmarkName)) {}
...
template <typename R>
struct StringMaker<
    R, std::enable_if_t<is_range<R>::value &&
...
#define INTERNAL_CATCH_CAPTURE(varName, macroName, ...)                        \
  Catch::Capturer varName(macroName, CATCH_INTERNAL_LINEINFO,                  \
...
public:
  explicit ReporterRegistrar(std::string const &name) {
    getMutableRegistryHub().registerReporter(
        name, Detail::make_unique<ReporterFactory<T>>());
  }
};
...
public:
  ListenerRegistrar() {
    getMutableRegistryHub().registerListener(
        Detail::make_unique<ListenerFactory>());
  }
};
} // namespace Catch
...
protected:
  ...
  BasicResult(ResultType type, std::string const &message)
...
public:
  ...
  parse(std::string const &, Detail::TokenStream const &tokens) const override;
};
...
#define INTERNAL_CATCH_TEST(macroName, resultDisposition, ...)                 \
  do {                                                                         \
    /* The expression should not be evaluated, but warnings should hopefully   \
     * be checked */                                                           \
...
Detail::unique_ptr<ITestInvoker> makeTestInvoker(void (C::*testAsMethod)()) {
  return Detail::unique_ptr<ITestInvoker>(
      new TestInvokerAsMethod<C>(testAsMethod));
}
...
struct TestCaseInfo : Detail::NonCopyable {
  ...
  TestCaseInfo(std::string const &_className, NameAndTags const &_tags,
...
makeTestCaseInfo(std::string const &className, NameAndTags const &nameAndTags,
                 SourceLineInfo const &lineInfo);
} // namespace Catch
...
public:
  ...
  GeneratorWrapper(IGenerator<T> *generator) : m_generator(generator) {}
...
public:
  SingleValueGenerator(T &&value) : m_value(std::forward<T>(value)) {}
...
template <typename T> GeneratorWrapper<T> value(T &&value) {
  return GeneratorWrapper<T>(
      Catch::Detail::make_unique<SingleValueGenerator<T>>(
          std::forward<T>(value)));
}
template <typename T>
...
template <typename T>
std::enable_if_t<std::is_floating_point<T>::value, GeneratorWrapper<T>>
random(T a, T b) {
  return GeneratorWrapper<T>(
...
public:
  MatchExpr(ArgT &&arg, MatcherT const &matcher, StringRef const &matcherString)
      : ITransientExpression{true,
                             matcher.match(
                                 arg)}, // not forwarding arg here on purpose
        m_arg(std::forward<ArgT>(arg)), m_matcher(matcher),
...
namespace Matchers {
template <typename ArgT> struct MatcherBase;
}
...
auto makeMatchExpr(ArgT &&arg, MatcherT const &matcher,
  ...
  return MatchExpr<ArgT, MatcherT>(std::forward<ArgT>(arg), matcher,
                                   matcherString);
}
...
template <typename ArgT> struct MatchAllOf final : MatcherBase<ArgT> {
  MatchAllOf() = default;
...
template <typename ArgT> struct MatchAnyOf final : MatcherBase<ArgT> {
  MatchAnyOf() = default;
...
template <typename ArgT> struct MatchNotOf final : MatcherBase<ArgT> {
...
template <typename... Cond> struct conjunction : std::true_type {};

template <typename Cond, typename... Rest>
struct conjunction<Cond, Rest...>
    : std::integral_constant<bool, Cond::value && conjunction<Rest...>::value> {
};
...
bool match_all_of(Arg &&, std::array<void const *, N> const &,
                  std::index_sequence<>) {
  return true;
}

template <typename T, typename... MatcherTs, std::size_t N, typename Arg,
          std::size_t Idx, std::size_t... Indices>
bool match_all_of(Arg &&arg, std::array<void const *, N> const &matchers,
  ...
  return static_cast<T const *>(matchers[Idx])->match(arg) &&
         match_all_of<MatcherTs...>(arg, matchers,
                                    std::index_sequence<Indices...>{});
}

template <std::size_t N, typename Arg>
bool match_any_of(Arg &&, std::array<void const *, N> const &,
                  std::index_sequence<>) {
  return false;
}
...
bool match_any_of(Arg &&arg, std::array<void const *, N> const &matchers,
                  std::index_sequence<Idx, Indices...>) {
  return static_cast<T const *>(matchers[Idx])->match(arg) ||
         match_any_of<MatcherTs...>(arg, matchers,
                                    std::index_sequence<Indices...>{});
}
...
struct MatchAllOfGeneric final : MatcherGenericBase {
  ...
  friend MatchAllOfGeneric<MatcherTs..., MatchersRHS...>
  operator&&(MatchAllOfGeneric<MatcherTs...> &&lhs,
...
struct MatchAnyOfGeneric final : MatcherGenericBase {
  ...
  template <typename... MatchersRHS>
  friend MatchAnyOfGeneric<MatcherTs..., MatchersRHS...>
  operator||(MatchAnyOfGeneric<MatcherTs...> &&lhs,
...
public:
  explicit SizeMatchesMatcher(Matcher m) : m_matcher(std::move(m)) {}
...
public:
  ...
  template <typename RangeLike> bool match(RangeLike &&rng) const {
    ...
    return end(rng) !=
           std::find_if(begin(rng), end(rng), [&](auto const &elem) {
...
public:
  ...
  // cases)
  ContainsMatcherMatcher(Matcher matcher) : m_matcher(std::move(matcher)) {}
...
public: // IStreamingReporter
  ...
  void listTests(std::vector<TestCaseHandle> const &tests,
                 IConfig const &config) override;
  void listTags(std::vector<TagInfo> const &tags,
                IConfig const &config) override;
};
...
struct SonarQubeReporter : CumulativeReporterBase {
  ...
  void writeTestFile(std::string const &filename,
                     TestGroupNode::ChildNodes const &testCaseNodes);

  void writeTestCase(TestCaseNode const &testCaseNode);

  void writeSection(std::string const &rootName, SectionNode const &sectionNode,
                    bool okToFail);
...
struct TAPReporter : StreamingReporterBase {
  ...
  TAPReporter(ReporterConfig const &config) : StreamingReporterBase(config) {
    m_preferences.shouldReportAllAssertions = true;
  }
